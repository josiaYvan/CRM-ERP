...
class MessagesController extends Controller
{
    ...
    public function download($fileName)
    {
        if (Chatify::storage()->exists(config('chatify.attachments.folder') . '/' . $fileName)) {
            ...
            return abort(404, "Désolé, le fichier n'existe pas dans le serveur ou a été supprimé!");
        }
    }
        ...
        if ($request->hasFile('file')) {
            ...
            if ($file->getSize() < Chatify::getMaxUploadSize()) {
                if (in_array(strtolower($file->getClientOriginalExtension()), $allowed)) {
                    ...
                    $error->status = 1;
                    $error->message = "Extension du fichier non supporté!";
                }
            } else {
                $error->status = 1;
                $error->message = "Fichier volumineux!";
            }
        }
    ...
    public function fetch(Request $request)
    {
        ...
        if ($totalMessages < 1) {
            $response['messages'] ='<p class="message-hint center-el"><span>Saluer pour commencer la conversation.</span></p>';
    ...
    public function getContacts(Request $request)
    {
        ...
        if (count($usersList) > 0) {
            ...
            $contacts = '<p class="message-hint center-el"><span>Votre contact est vide.</span></p>';
        }
    ...
    public function search(Request $request)
    {
        ...
        if($records->total() < 1){
            $getRecords = '<p class="message-hint center-el"><span>Rien à afficher.</span></p>';
        }
        // send the response
        return Response::json([
            'records' => $getRecords,
    ...
    public function sharedPhotos(Request $request)
    {
        ...
        // send the response
        return Response::json([
            'shared' => count($shared) > 0 ? $sharedPhotos : '<p class="message-hint"><span>Aucun fichier partagé.</span></p>',
        ], 200);
    }
        ...
        if ($request->hasFile('avatar')) {
            ...
            if ($file->getSize() < Chatify::getMaxUploadSize()) {
                if (in_array(strtolower($file->getClientOriginalExtension()), $allowed_images)) {
                    ...
                    $msg = "Extension du fichier non supporté!";
                    $error = 1;
                }
            } else {
